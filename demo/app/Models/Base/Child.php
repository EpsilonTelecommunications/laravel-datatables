...
abstract class Child implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\App\\Models\\Map\\ChildTableMap';
    ...
    public function setGenderId($v)
    {
        ...
        if ($this->gender_id !== $v) {
            $this->gender_id = $v;
            $this->modifiedColumns[ChildTableMap::COL_GENDER_ID] = true;
        }
    ...
    public function setName($v)
    {
        ...
        if ($this->name !== $v) {
            $this->name = $v;
            $this->modifiedColumns[ChildTableMap::COL_NAME] = true;
        }
    ...
    public function setDateOfBirth($v)
    {
        ...
        if ($this->date_of_birth !== null || $dt !== null) {
            if ($this->date_of_birth === null || $dt === null || $dt->format("Y-m-d H:i:s.u") !== $this->date_of_birth->format("Y-m-d H:i:s.u")) {
                ...
                $this->modifiedColumns[ChildTableMap::COL_DATE_OF_BIRTH] = true;
            }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : ChildTableMap::translateFieldName('GenderId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->gender_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : ChildTableMap::translateFieldName('Name', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 4; // 4 = ChildTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\App\\Models\\Child'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(ChildTableMap::COL_GENDER_ID)) {
            $modifiedColumns[':p' . $index++]  = 'gender_id';
        }
        if ($this->isColumnModified(ChildTableMap::COL_NAME)) {
            $modifiedColumns[':p' . $index++]  = 'name';
        }
        if ($this->isColumnModified(ChildTableMap::COL_DATE_OF_BIRTH)) {
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\App\Models\Child The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ChildTableMap::COL_GENDER_ID)) {
            $criteria->add(ChildTableMap::COL_GENDER_ID, $this->gender_id);
        }
        if ($this->isColumnModified(ChildTableMap::COL_NAME)) {
            $criteria->add(ChildTableMap::COL_NAME, $this->name);
        }
        if ($this->isColumnModified(ChildTableMap::COL_DATE_OF_BIRTH)) {
            $criteria->add(ChildTableMap::COL_DATE_OF_BIRTH, $this->date_of_birth);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \App\Models\Child (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \App\Models\Child Clone of current object.
     * @throws PropelException
     */
