...
abstract class Gender implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\App\\Models\\Map\\GenderTableMap';
    ...
    /**
     * @var        ObjectCollection|ChildAdult[] Collection to store aggregation of ChildAdult objects.
     */
    ...
    /**
     * Initializes internal state of App\Models\Base\Gender object.
     */
     ...
     * @return $this|\App\Models\Gender The current object (for fluent API support)
     */
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[GenderTableMap::COL_ID] = true;
        }
    ...
    public function setRaceId($v)
    {
        ...
        if ($this->race_id !== $v) {
            $this->race_id = $v;
            $this->modifiedColumns[GenderTableMap::COL_RACE_ID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : GenderTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : GenderTableMap::translateFieldName('RaceId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->race_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : GenderTableMap::translateFieldName('Name', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 3; // 3 = GenderTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\App\\Models\\Gender'), 0, $e);
        }
    }
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->adultsScheduledForDeletion !== null) {
                if (!$this->adultsScheduledForDeletion->isEmpty()) {
                    foreach ($this->adultsScheduledForDeletion as $adult) {
            ...
            if ($this->collAdults !== null) {
                foreach ($this->collAdults as $referrerFK) {
            ...
            if ($this->childrenScheduledForDeletion !== null) {
                if (!$this->childrenScheduledForDeletion->isEmpty()) {
                    foreach ($this->childrenScheduledForDeletion as $child) {
                        // need to save related object because we set the relation to null
                        $child->save($con);
                    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        $this->modifiedColumns[GenderTableMap::COL_ID] = true;
        if (null !== $this->id) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . GenderTableMap::COL_ID . ')');
        }

         // check the columns in natural order for more readable SQL queries
        if ($this->isColumnModified(GenderTableMap::COL_ID)) {
            $modifiedColumns[':p' . $index++]  = 'id';
        }
        if ($this->isColumnModified(GenderTableMap::COL_RACE_ID)) {
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            ...
            if (null !== $this->collAdults) {
                ...
                $result[$key] = $this->collAdults->toArray(null, false, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\App\Models\Gender The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(GenderTableMap::COL_ID)) {
            $criteria->add(GenderTableMap::COL_ID, $this->id);
        }
        if ($this->isColumnModified(GenderTableMap::COL_RACE_ID)) {
            $criteria->add(GenderTableMap::COL_RACE_ID, $this->race_id);
        }
        if ($this->isColumnModified(GenderTableMap::COL_NAME)) {
     ...
     * objects.
     *
     * @param      object $copyObj An object of \App\Models\Gender (or compatible) type.
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($deepCopy) {
            ...
            foreach ($this->getAdults() as $relObj) {
                if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
                    $copyObj->addAdult($relObj->copy($deepCopy));
                }
            }
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \App\Models\Gender Clone of current object.
     * @throws PropelException
     */
    ...
    {
        if ('Adult' == $relationName) {
            return $this->initAdults();
        }
    ...
    /**
     * Reset is the collAdults collection loaded partially.
     */
    public function resetPartialAdults($v = true)
    {
        $this->collAdultsPartial = $v;
    }

    /**
     * Initializes the collAdults collection.
     *
     * By default this just sets the collAdults collection to an empty array (like clearcollAdults());
     ...
     * @return void
     */
    public function initAdults($overrideExisting = true)
    {
        if (null !== $this->collAdults && !$overrideExisting) {
            return;
        }

        $collectionClassName = AdultTableMap::getTableMap()->getCollectionClassName();
    ...
    /**
     * Gets an array of ChildAdult objects which contain a foreign key that references this object.
     *
    ...
    public function getAdults(Criteria $criteria = null, ConnectionInterface $con = null)
    {
        ...
        if (null === $this->collAdults || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collAdults) {
                ...
                $collAdults = ChildAdultQuery::create(null, $criteria)
                    ->filterByGender($this)
                    ->find($con);

                if (null !== $criteria) {
                    if (false !== $this->collAdultsPartial && count($collAdults)) {
                        $this->initAdults(false);

                        foreach ($collAdults as $obj) {
                            if (false == $this->collAdults->contains($obj)) {
                                $this->collAdults->append($obj);
                            }
                        }
    ...
    /**
     * Sets a collection of ChildAdult objects related by a one-to-many relationship
    ...
    {
        /** @var ChildAdult[] $adultsToDelete */
        $adultsToDelete = $this->getAdults(new Criteria(), $con)->diff($adults);


        $this->adultsScheduledForDeletion = $adultsToDelete;

        foreach ($adultsToDelete as $adultRemoved) {
            $adultRemoved->setGender(null);
        }
    ...
    /**
     * Returns the number of related Adult objects.
     *
     * @param      Criteria $criteria
     * @param      boolean $distinct
     * @param      ConnectionInterface $con
     * @return int             Count of related Adult objects.
     * @throws PropelException
     */
    ...
    /**
     * Method called to associate a ChildAdult object to this object
     * through the ChildAdult foreign key attribute.
     *
     * @param  ChildAdult $l ChildAdult
     * @return $this|\App\Models\Gender The current object (for fluent API support)
     */
    public function addAdult(ChildAdult $l)
    {
        if ($this->collAdults === null) {
            $this->initAdults();
            $this->collAdultsPartial = true;
        ...
        if (!$this->collAdults->contains($l)) {
            $this->doAddAdult($l);

            if ($this->adultsScheduledForDeletion and $this->adultsScheduledForDeletion->contains($l)) {
                $this->adultsScheduledForDeletion->remove($this->adultsScheduledForDeletion->search($l));
            }
        }
    ...
    /**
     * @param ChildAdult $adult The ChildAdult object to add.
     */
    protected function doAddAdult(ChildAdult $adult)
    {
        $this->collAdults[]= $adult;
        $adult->setGender($this);
    }

    /**
     * @param  ChildAdult $adult The ChildAdult object to remove.
     * @return $this|ChildGender The current object (for fluent API support)
     */
    public function removeAdult(ChildAdult $adult)
    {
        if ($this->getAdults()->contains($adult)) {
            $pos = $this->collAdults->search($adult);
            $this->collAdults->remove($pos);
            ...
            $this->adultsScheduledForDeletion[]= $adult;
            $adult->setGender(null);
        }
